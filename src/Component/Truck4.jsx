...
// Update the Loader component to maintain full width
const Loader = () => (
  <div className="w-full min-w-full overflow-x-auto p-8">
...
// Main Truck4 component to manage form data and table
function Truck4() {
  ...
  const saveData = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        const docRef = doc(db, "Truck4", formData.id);
        ...
        await addDoc(collection(db, "Truck4"), {
          date: formData.date,
          driverName: formData.driverName,
          jobs: formData.jobs,
        });
  ...
  const fetchData = async () => {
    try {
      ...
      const querySnapshot = await getDocs(collection(db, "Truck4"));
  ...
  const handleDelete = async (id) => {
    if (!id) return;
    try {
      setLoading(true); // Set loading before delete
      await deleteDoc(doc(db, "Truck4", id));
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="p-8 bg-gray-100 min-h-screen min-w-[1024px]">
      ...
      {/* Only render chart when not loading */}
      {!loading && <ChartComponent chartData={prepareChartData()} />}
    </div>
  );
}

export default Truck4;
